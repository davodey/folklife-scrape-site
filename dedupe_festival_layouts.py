#!/usr/bin/env python3
"""
Layout duplicate detector for festival screenshots.

This script analyzes screenshots from the festival-screens-x folder to determine 
which images share the same layout. It normalizes images, optionally crops fixed 
top/bottom bars, optionally masks text via OCR, computes multiple layout fingerprints 
(perceptual hashes, edge/block signatures, and projection histograms), and clusters 
near-duplicates.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Cluster layout-duplicate festival screenshots")
    parser.add_argument("--input-dir", type=str, default="festival-screens-x", help="Directory of screenshots")
    parser.add_argument("--glob", type=str, default="*.png", help="Glob to match files (e.g., '*.png')")
    parser.add_argument("--output-csv", type=str, default="festival_layout_clusters.csv", help="Path to write results CSV")
    parser.add_argument("--clusters-dir", type=str, default="festival_layout_clusters", help="Optional directory to copy/symlink clusters into")
    parser.add_argument("--contact-sheets-dir", type=str, default="festival_layout_contact_sheets", help="Optional directory to write contact sheets per cluster")
...
def cosine_distance(u: np.ndarray, v: np.ndarray) -> float:
    try:
        ...
        if denom == 0:
            return 1.0
        result = 1.0 - float(np.dot(u, v) / denom)
        # Ensure result is finite and in valid range
        if not np.isfinite(result) or result < 0 or result > 2:
            return 1.0
        return result
    except Exception:
        return 1.0
...
def l1_distance(u: np.ndarray, v: np.ndarray) -> float:
    try:
        result = float(np.mean(np.abs(u - v)))
...
def combined_distance(a: ImageFeatures, b: ImageFeatures, alpha: float, beta: float, gamma: float) -> float:
    ...
    # Validate that all distances are finite and non-negative
    if not (np.isfinite(dh) and np.isfinite(de) and np.isfinite(dp)):
        return 1.0  # Return maximum distance for invalid cases
    ...
    result = alpha_n * dh + beta_n * de + gamma_n * dp
